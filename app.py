...
def home():
    ...
    if us.hasSession(session):
        us.clearSession(session)

    # Post method controleerd of de user op de start knop heeft gedrukt
    if request.method == "POST":
        form_name = request.form["name"]
        us.createSession(session, form_name)
...
def done():
    ...
    return render_template('done.html', name = us.getGebruikersnaam(session))
